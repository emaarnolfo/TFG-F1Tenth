...
{   
    ...
    // 2. Encontrar el punto más cercano
    ...
    // 4. Encontrar la longitud máxima del gap
    int gap_start = 0, gap_end = 0;
    gp::find_max_gap(filtered_ranges, gap_start, gap_end);
    
    // 5. Encontrar el mejor punto en el gap
    ...
    RCLCPP_INFO(this->get_logger(), "Ángulo de dirección: %.f deg", steering_angle * 180 / M_PI);

    // 7. Publicar mensaje de conducción
    publish_drive_msg(steering_angle);
    
}
/**
 * @brief Publica un mensaje AckermannDriveStamped con el ángulo de dirección y la velocidad adecuada.
 * La velocidad se ajusta en función del ángulo de dirección para garantizar una conducción segura. 
 */
